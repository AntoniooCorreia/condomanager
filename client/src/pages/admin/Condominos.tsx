...
import { Edit, Plus, Trash2, Users, Home, AlertCircle, CheckCircle2, Clock, Calendar, RepeatIcon } from "lucide-react";
    ...
    <div className="space-y-8">
      ...
      <Tabs defaultValue="condominos">
        <TabsList className="mb-6">
          <TabsTrigger value="condominos"><Home className="w-4 h-4 mr-2" />Proprietarios ({residents.length})</TabsTrigger>
          <TabsTrigger value="arrendatarios"><Users className="w-4 h-4 mr-2" />Arrendatarios ({arrendatarios.length})</TabsTrigger>
        </TabsList>
              ...
              <Table>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    <TableRow><TableCell colSpan={5} className="text-center py-10 text-muted-foreground">Nenhum utilizador encontrado.</TableCell></TableRow>
                  ) : residents.map((user) => (
                    <TableRow key={user.id} className="group border-border/50 hover:bg-secondary/20">
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          ...
                          {user.userType === "gestor" && <Badge className="bg-purple-50 text-purple-600 border-purple-200 w-fit">Administrador</Badge>}
                          {user.userType === "condomino" && <Badge className="bg-blue-50 text-blue-600 border-blue-200 w-fit">Proprietario</Badge>}
                        </div>
                      </TableCell>
            ...
            <div className="space-y-3">
              {arrendatarios.length === 0 ? (
                <Card className="p-8 text-center border-dashed">
                  ...
                  <p className="text-muted-foreground font-medium">Sem arrendatarios</p>
                </Card>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {condominoAssoc && <p className="text-xs text-primary">Proprietario: {condominoAssoc.name}</p>}
                          </div>
                        </div>
            ...
            <div className="lg:col-span-2">
              {!selectedTenant ? (
                <Card className="p-12 text-center border-dashed h-full flex flex-col items-center justify-center">
                  ...
                  <p className="text-muted-foreground font-medium">Selecione um arrendatario para ver os detalhes</p>
                </Card>
              ) : (() => {
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            {condominoAssoc && <p className="text-xs text-primary mt-1">Proprietario: {condominoAssoc.name} (Fracao {condominoAssoc.unit})</p>}
                          </div>
                        </div>
                          ...
                          {overdue.map(p => (
                            <div key={p.id} className="flex items-center justify-between text-sm">
                              ...
                              <div className="flex items-center gap-2">
                                <span className="text-rose-600 font-bold">EUR{Number(p.amount).toFixed(2)}</span>
