...
export function UserHome() {
  ...
  const myTenants = user?.id ? users?.filter(u => Number(u.relatedCondominoId) === user.id) || [] : [];
  ...
  const onSubmitSingle = (data: InsertPayment) => {
    createPayment.mutate(data, {
      onSuccess: () => {
        toast({ title: "Aviso enviado", description: "O arrendatario foi notificado." });
        setOpenSingle(false);
        singleForm.reset();
      },
    });
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <div className="relative z-10">
          <Badge className="bg-white/20 hover:bg-white/30 text-white border-none mb-4 backdrop-blur-sm">Fracao {user?.unit}</Badge>
          <h1 className="text-3xl sm:text-4xl font-display font-bold mb-2">Ola, {user?.name.split(" ")[0]}</h1>
          <p className="text-primary-foreground/80 max-w-lg">
            Bem-vindo a area do condomino. Aqui pode gerir pagamentos, efetuar reservas e aceder a informacoes do edificio.
          </p>
        </div>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
          <Card className="p-6 h-full border-border/50 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div><h3 className="font-bold text-lg">Pagamentos</h3><p className="text-sm text-muted-foreground">O seu estado financeiro</p></div>
            </div>
            {pendingPayment ? (
              <div className="bg-secondary/50 rounded-xl p-4 mb-6 border border-border">
                <p className="text-sm text-muted-foreground mb-1">Proximo pagamento:</p>
                <div className="flex justify-between items-end">
                  <div>
                    <p className="font-bold text-xl">EUR{pendingPayment.amount}</p>
              ...
              <div className="bg-emerald-50 rounded-xl p-4 mb-6 border border-emerald-100 flex items-center gap-3">
                ...
                <div><p className="font-bold text-emerald-800">Tudo em dia!</p><p className="text-sm text-emerald-600">Nao tem pagamentos pendentes.</p></div>
              </div>
            )}
            <Link href="/user/pagamentos" className="block w-full">
              <Button variant="outline" className="w-full justify-between hover:bg-secondary">Ver Historico <ArrowRight className="w-4 h-4" /></Button>
            </Link>
          </Card>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
          <Card className="p-6 h-full border-border/50 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div><h3 className="font-bold text-lg">Proxima Reserva</h3><p className="text-sm text-muted-foreground">As suas areas comuns</p></div>
            </div>
            {myNextRes ? (
              <div className="bg-secondary/50 rounded-xl p-4 mb-6 border border-border">
              ...
              <div className="bg-secondary/30 rounded-xl p-6 mb-6 text-center border border-border border-dashed">
                ...
                <p className="text-sm text-muted-foreground/70">Aproveite a piscina ou o ginasio do edificio.</p>
              </div>
            )}
            <Link href="/user/reservar" className="block w-full">
              <Button variant="outline" className="w-full justify-between hover:bg-secondary">Efetuar Reserva <ArrowRight className="w-4 h-4" /></Button>
            </Link>
          </Card>
        </motion.div>
      </div>
          ...
          <Card className="p-6 border-border/50">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-lg">Gestao de Arrendatarios</h3>
                  <p className="text-sm text-muted-foreground">{myTenants.length} arrendatario{myTenants.length > 1 ? "s" : ""} associado{myTenants.length > 1 ? "s" : ""}</p>
                </div>
              </div>
              <div className="flex gap-2">
                <Dialog open={openSingle} onOpenChange={setOpenSingle}>
                  <DialogTrigger asChild>
                    <Button variant="outline" size="sm"><SendIcon className="w-4 h-4 mr-2" /> Aviso Unico</Button>
                  </DialogTrigger>
                  <DialogContent>
                    <DialogHeader><DialogTitle>Enviar Aviso de Pagamento</DialogTitle></DialogHeader>
                    <Form {...singleForm}>
                      <form onSubmit={singleForm.handleSubmit(onSubmitSingle)} className="space-y-4">
                        ...
                        <FormField control={singleForm.control} name="description" render={({ field }) => (
                          <FormItem><FormLabel>Descricao</FormLabel><FormControl><Input placeholder="Ex: Renda Junho" {...field} /></FormControl><FormMessage /></FormItem>
                        )} />
                        <FormField control={singleForm.control} name="amount" render={({ field }) => (
                          <FormItem><FormLabel>Valor (EUR)</FormLabel><FormControl><Input type="number" min="0" step="0.01" {...field} /></FormControl><FormMessage /></FormItem>
                        )} />
                        ...
                        <Button type="submit" className="w-full" disabled={createPayment.isPending}>{createPayment.isPending ? "A enviar..." : "Enviar Aviso"}</Button>
                      </form>
                    </Form>
                  </DialogContent>
                </Dialog>
                ...
                <Dialog open={openSchedule} onOpenChange={setOpenSchedule}>
                  <DialogTrigger asChild>
                    <Button size="sm"><RepeatIcon className="w-4 h-4 mr-2" /> Agendamento Mensal</Button>
                  </DialogTrigger>
                  <DialogContent>
                    <DialogHeader><DialogTitle>Criar Agendamento Mensal</DialogTitle></DialogHeader>
                    <Form {...scheduleForm}>
                      <form onSubmit={scheduleForm.handleSubmit(onSubmitSchedule)} className="space-y-4">
                        ...
                        <FormField control={scheduleForm.control} name="amount" render={({ field }) => (
                          <FormItem><FormLabel>Valor Mensal (EUR)</FormLabel><FormControl><Input type="number" min="0" step="0.01" {...field} /></FormControl><FormMessage /></FormItem>
                        )} />
                        <FormField control={scheduleForm.control} name="dayOfMonth" render={({ field }) => (
                          <FormItem>
                            <FormLabel>Dia do Mes</FormLabel>
                            <Select onValueChange={(v) => field.onChange(parseInt(v))} defaultValue={field.value?.toString()}>
                              <FormControl><SelectTrigger><SelectValue placeholder="Dia do mes" /></SelectTrigger></FormControl>
                  ...
                  <div key={tenant.id} className="bg-secondary/30 rounded-xl p-4 border border-border">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-sm">{tenant.name.charAt(0)}</div>
                        <div><p className="font-bold text-sm">{tenant.name}</p><p className="text-xs text-muted-foreground">Fracao {tenant.unit}</p></div>
                      </div>
                      {tenantPending > 0 && <Badge className="bg-amber-50 text-amber-600 border-amber-200 text-xs">{tenantPending} pendente{tenantPending > 1 ? "s" : ""}</Badge>}
                    </div>
                      ...
                      <div className="mt-2 pt-2 border-t border-border/50">
                        {tenantSchedules.map(s => (
                          <div key={s.id} className="flex items-center justify-between text-xs text-muted-foreground">
                            ...
                            <span className="font-medium">EUR{s.amount}/mes dia {s.dayOfMonth}</span>
                          </div>
                        ))}
                      </div>
                    )}
